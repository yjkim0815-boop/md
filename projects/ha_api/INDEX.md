...
작성자: dominic
상태: 진행중
요약: 해피포인트 하이브리드 앱 API 서버 (앱 웹뷰 + REST API). Java8/Spring5.2/JSP(SiteMesh3)/MyBatis WAR — 외장 Tomcat 운영 레거시 모놀리식 (2026-08-05 영구 짝=ha_admin 상단 명시)
---

# 📇 ha_api 문서 인덱스

> 🔗 **영구 짝(pairing)**: 관리자(BO) = [`ha_admin`](../ha_admin/INDEX.md). 해피포인트 앱 서비스의 **런타임 서버(ha_api) ↔ 이를 운영·설정하는 백오피스(ha_admin)** 는 한 서비스의 양쪽 끝이다. **한쪽만 보고 판단 금지** — 도메인/데이터/이벤트 기능은 양쪽을 함께 본다. (이 관계는 `event-template` 태스크와 **독립인 상시 관계**로, 태스크가 완료·아카이브돼도 유지된다.)
>
> 🧩 **앱 내 탑재 서비스(느슨한 연결, 짝 아님)**: 해피포인트 앱(베이스 = ha_api) 안에 **완전히 독립된 시스템** [`ha_panel`](../ha_panel/INDEX.md)(설문 패널 "패널KOK/SURVEY KOK")이 웹뷰로 탑재돼 있다. ha_api 와 코드/DB/운영은 별개라 함께 판단하는 "짝"이 **아니고**, **용어 라우팅용**이다: **"패널"·"설문"** 이라고 하면 → `ha_panel`.
