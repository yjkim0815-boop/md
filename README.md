...
6. **실행 vs 확인 구분(중요)**: 사용자가 **명시적으로 실행을 지시**("~해줘/만들어줘/적용해줘/바꿔줘/진행해")할 때만 **실제 작업(파일 생성·수정·이동·명령 실행)**을 수행한다.
   ...
   - **🚨 "계획 세워 / 보여줘" = 코드 수정 절대 금지, 계획만 (해제 불가, 사용자 지시 2026-08-06)**: 사용자가 **"계획 세워줘", "계획 짜봐", "보여줘", "검토해줘", "어떻게 할지 정리해줘"** 라고 하면 **파일 수정·생성·이동·명령을 절대 하지 말고**, 무엇을·어떤 순서로·어떤 파일을 손댈지 **작업계획만 쭉 세워서 제시**한다. 그리고 **"이대로 진행할까요?"** 로 승인받은 뒤에야 실제 코드를 수정한다. ⚠️ **과거에 "계획 세워"라고 했는데 바로 코드를 수정한 위반 사례가 여러 번 있었음 — 반복 금지.**
...
>
> ⏱️ **재주입 쓰로틀(2026-08-06 · 토큰 절감)**: `startup`/`clear` 는 항상 전량 주입하되, **`resume`/`compact` 는 3시간에 1회만** 재주입한다(그 사이엔 1줄 생략 마커만). 마지막 주입 시각은 `~/.happypoint-md-inject.last` 에 기록. 긴 세션의 compaction/resume 반복 주입(≈72KB×N)을 제거. `~/.claude`·`~/.codex` 훅 **byte-for-byte 동일**.
>   - **트리거 판별은 하네스 필드명에 의존하지 않는다**: Claude 는 stdin `source`, Codex 의 `source` 는 클라이언트(예 `"vscode"`)일 수 있어 **'키'가 아니라 '값'** 으로 `startup/resume/clear/compact` 를 찾는다. 못 찾으면(불명) **안전하게 주입**.
>   - **Codex 실포맷 검증용**: 매 실행 시 원 stdin 을 `~/.happypoint-md-inject.laststdin` 에 남긴다 → 코덱스 세션 1회 후 이 파일로 실제 트리거 필드 확인·튜닝 가능(민감정보 아님, 추후 제거 가능).
...
### 🛑 절대 원칙 (2026-08-06 확정 · 사용자 지시)

> **Claude Code에 적용한 것은 Codex에도 적용한다. 역도 같다.**
> 한쪽에만 넣고 끝내지 않는다. 하네스 제약으로 불가능하면 **작업 전에** 알리고, 사후에 "공통 적용 완료"라고 말하지 않는다.

- 전역 지침·훅·스킬·설정 중 **어느 하나라도** 한쪽에 넣으면, 같은 턴 안에서 반대쪽 반영까지 끝낸다.
- 반영 후 **아래 정합성 표를 실제로 확인**하고 결과를 보고한다. 확인 없이 "적용했다"고 답하지 않는다.
- 스크립트 본체는 **한 벌만 두고 양쪽에서 같은 경로를 호출**하는 것을 우선한다(사본 2개는 드리프트가 난다). 사본이 불가피하면 해시 일치를 확인한다.

#### 정합성 표 (변경 시마다 확인)

| 항목 | Claude Code | Codex | 동기화 방식 |
|---|---|---|---|
| 전역 지침 | `~/.claude/CLAUDE.md` | `~/.codex/AGENTS.md` | 문구 동일. 경로 표기만 각자 |
| 훅 등록 | `~/.claude/settings.json` → `hooks` | `~/.codex/hooks.json` → `hooks` | 이벤트 구성 동일 |
| KB 자동주입 | `~/.claude/hooks/inject-readme.js` | `~/.codex/hooks/inject-readme.js` | **사본 2개 · 해시 일치 필수** |
| 현행화 리마인더 | `~/.claude/hooks/remind-worklog.js` | 좌동(같은 경로 호출) | 단일 파일 공유 |
| 활동 수집(자비스) | `~/.claude/hooks/jarvis.js` | 좌동(같은 경로 호출) | 단일 파일 공유 |

- 지원 훅 이벤트는 양쪽 동일하다: `SessionStart` `SessionEnd` `UserPromptSubmit` `PreToolUse` `PostToolUse` `Stop` `SubagentStop` `Notification` `PreCompact`.
- **하네스별 차이(공통 적용 불가)**: 메모리 체계는 Claude가 파일(`.claude/.../memory/`), Codex가 SQLite(`memories_1.sqlite`)로 서로 다르다. MCP 서버 등록도 `.claude.json` vs `config.toml`로 형식이 다르다. 이 둘은 **각자 방식으로 동등하게** 맞춘다.
- 설정 파일을 고치기 전에 **백업**한다(`hooks.json.bak-YYYYMMDD` 형식). 훅은 세션 시작 시 읽히므로 **적용하려면 해당 하네스를 재시작**해야 한다.
...
- **🚨 코드 수정 = 자동으로 "배포 대상 + 테스트 대상" (해제 불가, 사용자 지시 2026-08-06)**: 사용자가 "배포할게/배포해"라고 **말하지 않아도**, Claude 가 코드를 수정하면 그 변경은 **당연히 배포 대상이자 검증(테스트) 대상**이다. 별도 지시를 기다리지 말고 **즉시 `⏳배포대기` 로 기록**하고, 그 사실을 **세션 내내 기억**한다. 사용자가 **"배포만 하고 테스트 안 된 목록", "미검증 목록", "투두 보여줘"** 등을 물으면 **이 기억을 근거로 정확히 나열**한다(내가 수정한 것을 "사용자가 배포 안 했으니 목록에서 빠진다"고 처리 금지). 배포·확인 완료가 사용자 피드백으로 들어오면 `✅` 로 승격한다.
